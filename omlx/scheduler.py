...
def _warn_if_auxiliary_state_dropped(cache_obj: Any) -> None:
    ...
    Still open as of upstream 0.6.4. That release added a
    ``preserve_auxiliary_kv_state = True`` marker to both QSA cache classes
    but nothing reads it yet, so the drop is unchanged -- re-check whether
    this warning is still needed when upstream starts consuming the flag.
    """
